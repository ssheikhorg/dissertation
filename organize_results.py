...
def organize_downloaded_results(source_dir=".", target_dir="evaluation_results"):
    ...
    result_patterns = ["*_comprehensive_results.json", "*_ui_export_data.json", "*_results.json"]

    viz_patterns = ["*_chart.png", "comparison_table.html", "radar_chart.png"]
    ...
    for pattern in viz_patterns:
        for file_path in Path(source_dir).glob(pattern):
            if file_path.is_file():
                shutil.move(str(file_path), os.path.join("app/visualizations", file_path.name))
