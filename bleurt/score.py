...
def _get_default_checkpoint():
    ...
    assert tf.io.gfile.exists(ckpt), "Default checkpoint not found! Are you sure the install is complete?"
    return ckpt
...
class LazyPredictor(Predictor):
    ...
    def __init__(self, checkpoint):
        ...
        logging.warn(
            "Using Tensorflow Sessions---please call `.close()` when you are are done using the BleurtScorer."
        )
...
class BleurtScorer:
    ...
    def __init__(self, checkpoint=None, predict_fn=None):
        ...
        logging.info(f"Reading checkpoint {checkpoint}.")
    ...
    def score(self, *args, references=[], candidates=[], batch_size=None):
        ...
        assert len(candidates) == len(references), (
            "The number of candidate sentences must match the number of reference sentences."
        )
        if not candidates:
            return []
...
class LengthBatchingBleurtScorer(BleurtScorer):
    ...
    def score(self, *args, references=[], candidates=[], batch_size=None):
        ...
        for i in range(0, n_items, batch_size):
            ...
            # Runs the inference.
            tf_input = {"input_ids": batch_input_ids, "input_mask": batch_input_mask, "segment_ids": batch_segment_ids}
            predict_out = self._predictor.predict(tf_input)
...
class SavedModelBleurtScorer:
    ...
    def __init__(self, saved_model=None, serialize_input=True):
        ...
        logging.info(f"Reading checkpoint {saved_model}.")
        ...
        for i in range(0, len(candidates), batch_size):
            ...
            if self.serialize_input:
                ...
                predict_out = self.bleurt_model_ops(
                    references=tf.constant(batch_ref), candidates=tf.constant(batch_cand)
                )["predictions"].numpy()
...
def create_bleurt_preprocessing_ops(tokenizer, max_seq_length):
    ...
    def _py_encode(references, candidates):
        input_ids, input_mask, segment_ids = encoding.encode_batch(references, candidates, tokenizer, max_seq_length)
        return input_ids, input_mask, segment_ids

    def bleurt_preprocessing_ops(references, candidates):
        """Builds a computation graph for BLEURT tokenization and encoding."""
        return tf.numpy_function(func=_py_encode, inp=[references, candidates], Tout=(tf.int64, tf.int64, tf.int64))
...
def create_bleurt_ops(checkpoint=None, bleurt_model_fn=None):
    ...
    assert bleurt_model_fn or tf.io.gfile.exists(checkpoint), "invalid path '%s'" % checkpoint
    ...
    def bleurt_ops(*args, references=None, candidates=None):
        ...
        logging.info(f"Reading info from checkpoint {checkpoint}")
        ...
        tokenizer = tokenizers.create_tokenizer(vocab_file=vocab_file, do_lower_case=do_lower_case, sp_model=sp_model)
        logging.info("Tokenizer created")
        logging.info("Creating BLEURT Preprocessing Ops...")
        bleurt_preprocessing_ops = create_bleurt_preprocessing_ops(tokenizer, max_seq_length)
        ...
        input_ids, input_mask, segment_ids = bleurt_preprocessing_ops(references, candidates)
        out = bleurt_model_ops(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids)
