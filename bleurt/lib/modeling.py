...
def layer_norm(input_tensor, name=None):
    ...
    return tf_slim.layer_norm(inputs=input_tensor, begin_norm_axis=-1, begin_params_axis=-1, scope=name)
...
def dense_layer_3d(input_tensor, num_attention_heads, size_per_head, initializer, activation, name=None):
    ...
    with tf.variable_scope(name):
        w = tf.get_variable(
            name="kernel", shape=[last_dim, num_attention_heads * size_per_head], initializer=initializer
        )
        w = tf.reshape(w, [last_dim, num_attention_heads, size_per_head])
        b = tf.get_variable(name="bias", shape=[num_attention_heads * size_per_head], initializer=tf.zeros_initializer)
...
def dense_layer_3d_proj(input_tensor, hidden_size, num_attention_heads, head_size, initializer, activation, name=None):
    ...
    """
    with tf.variable_scope(name):
        w = tf.get_variable(name="kernel", shape=[hidden_size, hidden_size], initializer=initializer)
...
def dense_layer_2d(input_tensor, output_size, initializer, activation, name=None):
    ...
    last_dim = get_shape_list(input_tensor)[-1]
    with tf.variable_scope(name):
        w = tf.get_variable(name="kernel", shape=[last_dim, output_size], initializer=initializer)
...
def attention_layer(
    ...
    query_layer = dense_layer_3d(
        from_tensor, num_attention_heads, size_per_head, create_initializer(initializer_range), query_act, "query"
    )

    # `key_layer` = [B, T, N, H]
    key_layer = dense_layer_3d(
        to_tensor, num_attention_heads, size_per_head, create_initializer(initializer_range), key_act, "key"
    )

    # `value_layer` = [B, T, N, H]
    value_layer = dense_layer_3d(
        to_tensor, num_attention_heads, size_per_head, create_initializer(initializer_range), value_act, "value"
    )
        ...
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.variable_scope("output"):
                layer_output = dense_layer_2d(
                    intermediate_output, hidden_size, create_initializer(initializer_range), None, "dense"
                )
