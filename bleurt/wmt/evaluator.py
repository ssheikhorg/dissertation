...
flags.DEFINE_string("candidate_checkpoint", None, "Path to BLEURT bleurt_checkpoint to benchmark.")
...
flags.DEFINE_string("test_file", None, "Path to JSONL ratings file to be used as test data.")

flags.DEFINE_string("results_json", None, "JSON file where the results will be written.")

flags.DEFINE_integer("sample_size", None, "Samples N items from the human ratings without replacement.")
...
def grouped_wmt_kendall(df, year=2019, threshold=25):
    ...
    tf.logging.debug(f"Subset size: {len(df.index)}")
    n_sentences = df["reference"].nunique()
    tf.logging.debug(f"Number of reference sentences: {n_sentences}")
    ...
    for _, group_df in groups:
        ...
        n_kept_groups += 1
        local_agreement, local_n_pairs, local_n_skipped = wmt_kendall(group_df, year, threshold, return_counts=True)
    ...
    if n_pairs == 0:
        tf.logging.info(
            "Zero pairs found, skipping. If this behavior is unexpected, please ensure that raw ratings exist exist."
        )
        return None

    tf.logging.debug(f"Found {agreement} agreements among {n_pairs} pairs")
    tf.logging.debug(f"{n_skipped} pairs skipped")
    tf.logging.debug(f"{n_kept_groups} groups n_kept_groups")
    tf.logging.debug(f"{n_skipped_groups} groups skipped")
    ...
    for i in range(n_items - 1):
        for j in range(i + 1, n_items):
            ...
            if raw_ratings[i] < raw_ratings[j]:
                ...
                raise ValueError(f"Wrong raw_ratings values: {raw_ratings[i]}, {raw_ratings[j]}")
            ...
            if predictions[i] < predictions[j]:
                ...
                raise ValueError(f"Wrong prediction values: {predictions[i]}, {predictions[j]}")
...
METRICS = {"kendall": kendall, "pearson": pearson, "spearman": spearman, "wmt_da_rr_kendall": grouped_wmt_kendall}
...
def eval_checkpoint(export_dir, test_file, results_json=None):
    ...
    def _scoring_fun(test_df):
        ...
        return scorer.score(references=test_df.reference.tolist(), candidates=test_df.candidate.tolist())
...
def eval_tf_export(export_dir, test_file, results_json=None):
    ...
    def _scoring_fun(test_df):
        scores = bleurt_scorer.score(references=test_df.reference, candidates=test_df.candidate)
        return scores

    return run_eval(_scoring_fun, test_file, results_json)


def eval_prediction_file(prediction_file, test_file, results_json=None, wmt_format=True, exclude_sys=None):
    """Runs evaluation on a prediction file, possibly in WMT format."""
    tf.logging.info(f"Evaluating file {prediction_file}")
    ...
    def _predict_from_file(test_data_df, filter_newstest=True):
        ...
        if wmt_format:
            ...
            col_names = ["metric", "lang", "corpus", "system", "segment_id", "prediction", "ens", "url"]
            predictions_df = pd.read_csv(tf.gfile.Open(prediction_file), sep="\t", names=col_names)
            if filter_newstest:
                predictions_df = predictions_df.loc[predictions_df["corpus"].str.startswith("newstest"), :]
        else:
        ...
        join_df = test_data_df.merge(predictions_df, how="left", on=["system", "segment_id", "lang"], sort=False)
        assert join_df["score"].count() == test_data_df["score"].count()
        assert np.all(
            (join_df["score"].to_numpy() == test_data_df["score"].to_numpy()) | np.isnan(join_df["score"].to_numpy())
        )

        predictions = join_df["prediction"]
        n_nulls = predictions.isna().sum()
        if n_nulls > 0:
            tf.logging.warning(f"Found {n_nulls} nulls in baseline")
...
def run_eval(scoring_fun, test_file, results_json=None, exclude_sys=None):
    ...
    if FLAGS.sample_size:
        logging.info(f"Sampling {str(FLAGS.sample_size)} items.")
    ...
    if exclude_sys:
        tf.logging.info(f"Excluding systems matching: {exclude_sys}.")
        tf.logging.info(f"Num rows before: {len(test_df.index)}.")
        test_df = test_df[~test_df["system"].str.match(exclude_sys)]
        tf.logging.info(f"Num rows after: {len(test_df.index)}.")
    ...
    for group_name, group_df in grouped_by_lang:
        ...
        grouped_by_system = group_df.groupby("system").agg({"bleurt": "mean", "score": "mean"}).reset_index()
        ...
        for metric_name in ["kendall", "pearson", "spearman"]:
            metric_value = METRICS[metric_name](predictions, reference)
            logging.info(f"** sys-{metric_name}: {metric_value}")
        ...
        grouped_by_system_nooutl = grouped_by_system[~grouped_by_system["system"].isin(outliers)]
        predictions = grouped_by_system_nooutl["bleurt"].to_numpy()
        reference = grouped_by_system_nooutl["score"].to_numpy()
        for metric_name in ["kendall", "pearson", "spearman"]:
            metric_value = METRICS[metric_name](predictions, reference)
            logging.info(f"** sys-{metric_name}-nooutl: {metric_value}")
        ...
        for metric_name in ["kendall", "pearson", "spearman"]:
            metric_value = METRICS[metric_name](predictions, reference)
            logging.info(f"** sys-{metric_name}-top10: {metric_value}")
...
def main(_):
    ...
    if FLAGS.candidate_predictions_file:
        eval_prediction_file(FLAGS.candidate_predictions_file, FLAGS.test_file, FLAGS.results_json)
