...
flags.DEFINE_string(
    "wmt_cache_dir",
    None,
    "[optional] Directory where downloads from the WMT Metrics Shared task websites will be cached.",
)

WMT_LOCATIONS = {
    2015: {
        "eval_data": ("DAseg-wmt-newstest2015", "DAseg-wmt-newstest2015.tar.gz", "https://www.scss.tcd.ie/~ygraham/")
    },
    2016: {
        "eval_data": ("DAseg-wmt-newstest2016", "DAseg-wmt-newstest2016.tar.gz", "https://www.scss.tcd.ie/~ygraham/")
    },
    2017: {
        "submissions": (
    ...
    2018: {
        ...
        "eval_data": ("newstest2018-humaneval", "newstest2018-humaneval.tar.gz", "https://www.scss.tcd.ie/~ygraham/"),
    },
    2019: {
        "submissions": (
        ...
        "eval_data": ("newstest2019-humaneval", "newstest2019-humaneval.tar.gz", "https://www.scss.tcd.ie/~ygraham/"),
    },
}
...
class WMTImporter:
    ...
    def fetch_files(self):
        ...
        if cache and not tf.io.gfile.exists(cache):
            logging.info(f"Initializing cache {cache}")
        ...
        for file_type in self.location_info:
            ...
            if cache:
                ...
                logging.info(f"Checking cache {cache_path}.")
            ...
            _ = tf.keras.utils.get_file(archive_name, url, cache_subdir=self.temp_directory, extract=True)
            logging.info("Done")
            if cache:
                logging.info(f"Writing to cache {cache_path}.")
...
class Importer1516(WMTImporter):
    ...
    def list_files_for_lang(self, lang):
        ...
        year = self.year
        source_file = f"DAseg.newstest{str(year)}.source.{lang}"
        reference_file = f"DAseg.newstest{str(year)}.reference.{lang}"
        candidate_file = f"DAseg.newstest{str(year)}.mt-system.{lang}"
        rating_file = f"DAseg.newstest{str(year)}.human.{lang}"
    ...
    def generate_records_for_lang(self, lang):
        ...
        with (
            open(input_files["source"], encoding="utf-8") as source_file,
            open(input_files["reference"], encoding="utf-8") as reference_file,
            open(input_files["candidate"], encoding="utf-8") as candidate_file,
            open(input_files["rating"], encoding="utf-8") as rating_file,
        ):
            ...
            with open(self.target_file, "a+") as dest_file:
                for source, reference, candidate, rating in itertools.zip_longest(
                    ...
                    example = Importer1516.to_json(
                        year, lang, source, reference, candidate, rating, n_records + 1, None
                    )
                    dest_file.write(example)
                    dest_file.write("\n")
                    n_records += 1
            logging.info(f"Processed {str(n_records)} records")
            return n_records
...
class Importer17(WMTImporter):
    ...
    def fetch_files(self):
        ...
        logging.info(f"Unpacked the segments to {self.segments_path()}.")
    ...
    def get_ref_segments(self, lang):
        ...
        src_file = f"newstest2017-{src_lang}{tgt_lang}-src.{src_lang}"
        ref_file = f"newstest2017-{src_lang}{tgt_lang}-ref.{tgt_lang}"
        src_path = os.path.join(src_subfolder, src_file)
        ref_path = os.path.join(ref_subfolder, ref_file)

        logging.info(f"Reading data from files {src_path} and {ref_path}")
        ...
        logging.info(f"Read {len(src_segments)} source and {len(ref_segments)} reference segments.")
    ...
    def get_sys_segments(self, lang):
        ...
        logging.info(f"Reading submission files from {folder}")
        ...
        for sys_file_name in all_files:
            ...
            with open(sys_path, encoding="utf-8") as f_sys:
                sys_lines = f_sys.readlines()
                sys_lines = [postprocess_segment(s) for s in sys_lines]
                sys_segments[sys_name] = sys_lines

        logging.info(f"Read submissions from {len(sys_segments.keys())} systems")
        return sys_segments
    ...
    def get_raw_rating_scores(self, lang):
        ...
        raw_rating_path = os.path.join(
            self.temp_directory,
            folder_name,
            f"anon-proc-hits-seg-{lang[-2:]}",
            "analysis",
            "ad-seg-scores.csv.gz",
        )
        logging.info(f"Reading raw ratings from {raw_rating_path}")
    ...
    def generate_records_for_lang(self, lang):
        ...
        logging.info(f"Reading file {ratings_file_path}")
        n_records = 0
        with open(ratings_file_path, encoding="utf-8") as f_ratings:
            with open(self.target_file, "a+") as dest_file:
                for line in itertools.islice(f_ratings, 1, None):
                    for parsed_line in self.parse_rating(line):
                        ...
                        dest_file.write("\n")
                        n_records += 1
        logging.info(f"Done reading ratings file. {n_records} records written.")
        return n_records
...
class Importer18(WMTImporter):
    ...
    def get_ref_segments(self, lang):
        ...
        src_lang, tgt_lang = separate_lang_pair(lang)
        src_file = f"newstest2018-{src_lang}{tgt_lang}-src.{src_lang}"
        ref_file = f"newstest2018-{src_lang}{tgt_lang}-ref.{tgt_lang}"
    ...
    def get_ratings_path(self, lang):
        ...
        subfolder = "analysis"
        file_name = f"ad-seg-scores-{lang}.csv"
    ...
    @staticmethod
    def to_json(year, lang, src_segment, ref_segment, sys_segment, raw_score, z_score, seg_id, sys_name, n_ratings=0):
        """Converts record to JSON."""
        json_dict = {
            "year": year,
            "lang": lang,
...
class Importer19(Importer18):
    ...
    def get_ratings_path(self, lang):
        ...
        for cand_file in all_files:
            if cand_file.endswith(file_name):
                return cand_file
        raise ValueError(f"Can't find ratings for lang {lang}")
    ...
    def get_ref_segments(self, lang):
        ...
        src_lang, tgt_lang = separate_lang_pair(lang)
        src_file = f"newstest2019-{src_lang}{tgt_lang}-src.{src_lang}"
        ref_file = f"newstest2019-{src_lang}{tgt_lang}-ref.{tgt_lang}"
        src_path = os.path.join(self.temp_directory, folder, src_subfolder, src_file)
        ref_path = os.path.join(self.temp_directory, folder, ref_subfolder, ref_file)
