...
flags.DEFINE_spaceseplist(
    "rating_years",
    "2015 2016",
    'Years for which the ratings will be downloaded, between 2015 and 2019. Ex: "2015 2016 2017".',
)

flags.DEFINE_string(
    "target_language", "en", 'Two letter code for the target language. Ex: "en".A star `*` means all languages.'
)

flags.DEFINE_string("target_file", "/tmp/wmt_eval.jsonl", "Path to JSONL ratings file to be created.")
...
flags.DEFINE_string(
    "wmt_ratings_file", None, "[optional] Path where raw WMT ratings will be stored. Creates a temp file if None."
)

flags.DEFINE_string(
    "temp_directory", None, "[optional] Temporary directory where WMT archives will be downladed and untared."
)

# Post-processing.
flags.DEFINE_float("dev_ratio", None, "Ratio of data allocated to dev set. No split if None")

flags.DEFINE_bool(
    "prevent_leaks",
    True,
    "Prevent leaks when splitting, i.e., train and dev examples with the same reference sentence.",
)
...
def create_wmt_dataset(target_file, rating_years, target_language):
    ...
    assert target_file
    assert not os.path.exists(FLAGS.target_file), "Target file already exists. Aborting."
    assert rating_years, "No target year detected."
    for year in rating_years:
        assert year in WMT_IMPORTERS, f"No importer for year {year}."
    ...
    with tempfile.TemporaryDirectory(dir=FLAGS.temp_directory) as tmpdir:
        logging.info(f"Using tmp directory: {tmpdir}")

        n_records_total = 0
        for year in rating_years:
            logging.info(f"\nProcessing ratings for year {year}")
            ...
            for lang_pair in lang_pairs:
                if target_language != "*" and not lang_pair.endswith(target_language):
                    logging.info(f"Skipping language pair {lang_pair}")
                    continue

                logging.info(f"Generating records for {year} and language pair {lang_pair}")
                n_records = importer.generate_records_for_lang(lang_pair)
                logging.info(f"Imported {str(n_records)} records.")
                n_records_total += n_records

        logging.info(f"Done processing {n_records_total} elements")
...
def postprocess(target_file, remove_null_refs=True, average_duplicates=True):
    ...
    logging.info(f"\n*** Post-processing WMT ratings {target_file}.")
...
def _shuffle_no_leak(all_ratings_df, n_train):
    ...
    while is_split_leaky(split_ix):
        split_ix += 1
    if n_train != split_ix:
        logging.info(f"Moved split point from {n_train} to {split_ix} to prevent sentence leaking")
...
def shuffle_split(ratings_file, train_file=None, dev_file=None, dev_ratio=0.1, prevent_leaks=True):
    ...
    logging.info(f"Will attempt to set aside {n_rows - n_train} out of {n_rows} rows for dev.")
    if prevent_leaks:
        train_df, dev_df = _shuffle_no_leak(ratings_df, n_train)
    else:
        train_df, dev_df = _shuffle_leaky(ratings_df, n_train)
    logging.info(f"Created train and dev files with {len(train_df)} and {len(dev_df)} records.")
...
def main(_):
    ...
    if FLAGS.dev_ratio:
        shuffle_split(FLAGS.target_file, dev_ratio=FLAGS.dev_ratio, prevent_leaks=FLAGS.prevent_leaks)
