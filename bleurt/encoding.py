...
flags.DEFINE_string(
    "vocab_file",
    None,
    "Vocabulary file for WordPiece tokenization. Overridden if `sentence_piece_model` is specified.",
)
...
def serialize_example(reference, candidate, tokenizer, max_seq_length, score=None):
    ...
    input_ids, input_mask, segment_ids = encode_example(reference, candidate, tokenizer, max_seq_length)
...
def encode_batch(references, candidates, tokenizer, max_seq_length):
    ...
    """
    encoded_examples = []
    for ref, cand in zip(references, candidates, strict=False):
    ...
    return (stacked_examples[:, 0, :], stacked_examples[:, 1, :], stacked_examples[:, 2, :])


def encode_and_serialize(input_file, output_file, vocab_file, do_lower_case, sp_model, max_seq_length):
    ...
    for col in ["reference", "candidate", "score"]:
        assert col in examples_df.columns, f"field {col} not found in input file!"
    n_records = len(examples_df)
    logging.info(f"Read {n_records} examples.")
    ...
    with tf.python_io.TFRecordWriter(output_file) as writer:
        ...
        for record in examples_df.itertuples(index=False):
            ...
            tf_example = serialize_example(
                record.reference, record.candidate, tokenizer, max_seq_length, score=record.score
            )
            writer.write(tf_example)
    logging.info(f"Done writing {n_records} tf examples.")
