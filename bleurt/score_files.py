...
flags.DEFINE_string("candidate_file", None, "Path to candidates text file, with one candidate sentence per line.")

flags.DEFINE_string("reference_file", None, "Path to reference text file, with one reference sentence per line.")

flags.DEFINE_string(
    "scores_file", None, "[optional] Path where the scores will be written. Will use standard output if unspecified."
)
...
flags.DEFINE_integer(
    "read_buffer_size",
    100000,
    "Number of lines to read at a time from the input files. Increase or decrase to ajust memory consumption.",
)

flags.DEFINE_bool("batch_same_length", False, "Enables dynamic batching to speed up inference. [experimental feature]")


def _json_generator(sentence_pairs_file):
    """Yields a generator for iterating from a single JSONL file."""
    assert tf.io.gfile.exists(sentence_pairs_file), f"Sentence pairs file {sentence_pairs_file} not found"
    with tf.io.gfile.GFile(sentence_pairs_file, "r") as pairs_file:
        ...
        for _, row in ratings_df.iterrows():
            assert row.get("reference") is not None, (
                "Reference sentence not found, are you sure the JSON record contains a 'reference' field?"
            )
            assert row.get("candidate") is not None, (
                "Candidate sentence not found, are you sure the JSON record contains a 'candidate' field?"
            )
...
def _text_generator(reference_file, candidate_file):
    ...
    assert tf.io.gfile.exists(reference_file), f"Reference file {reference_file} not found"
    assert tf.io.gfile.exists(candidate_file), f"Candidate file {candidate_file} not found"
    with tf.io.gfile.GFile(reference_file, "r") as ref_file:
        with tf.io.gfile.GFile(candidate_file, "r") as cand_file:
            for ref_sentence, cand_sentence in itertools.zip_longest(ref_file, cand_file, fillvalue=None):
                assert ref_sentence is not None, (
                    "Reference sentence not found, are you sure that the files have the same size?"
                )
                assert cand_sentence is not None, (
                    "Candidate sentence not found, are you sure that the files have the same size?"
                )
...
def score_files(generator, bleurt_checkpoint):
    ...
    if not FLAGS.batch_same_length:
        ...
        logging.warning("Enabling same length batching. BEWARE: this is an experimental feature.")
        scorer = score_lib.LengthBatchingBleurtScorer(bleurt_checkpoint)

    def _consume_buffer():
        scores = scorer.score(references=ref_buffer, candidates=cand_buffer, batch_size=FLAGS.bleurt_batch_size)
        del ref_buffer[:]
        del cand_buffer[:]
...
def check_flags_and_score():
    ...
    assert FLAGS.sentence_pairs_file or (FLAGS.reference_file and FLAGS.candidate_file), (
        "Reference and candidate files not found, please specify a JSONL file or two text files."
    )
    if FLAGS.sentence_pairs_file:
        sentence_pairs_generator = _json_generator(FLAGS.sentence_pairs_file)
    else:
        sentence_pairs_generator = _text_generator(FLAGS.reference_file, FLAGS.candidate_file)
