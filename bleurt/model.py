...
from bleurt.lib import modeling, optimization
...
# BLEURT flags.
flags.DEFINE_string("bleurt_checkpoint_name", "bert_custom", "Name of the BLEURT export to be created.")

flags.DEFINE_string("init_bleurt_checkpoint", None, "Existing BLEURT export to be fine-tuned.")
...
flags.DEFINE_bool("dynamic_seq_length", True, "Exports model with dymaic sequence length.")

# Flags to control training setup.
flags.DEFINE_enum(
    "export_metric", "kendalltau", ["correlation", "kendalltau"], "Metric to chose the best model in export functions."
)

flags.DEFINE_integer("shuffle_buffer_size", 500, "Size of buffer used to shuffle the examples.")

# Flags to contol optimization.
flags.DEFINE_enum("optimizer", "adam", ["adam", "sgd", "adagrad"], "Which optimizer to use.")
...
flags.DEFINE_bool("use_ranking_loss", False, "Whether to use a ranking loss instead of regression (l2 loss).")

# BLEURT model flags.
flags.DEFINE_integer("n_hidden_layers", 0, "Number of fully connected/RNN layers before prediction.")
...
def create_model(
    ...
    logging.info(f"BERT embedding width: {str(bert_embed_size)}")
    ...
    for i in range(n_hidden_layers):
        ...
        logging.info(f"Adding hidden layer {i + 1}")
        output_layer = tf.layers.dense(output_layer, hidden_layers_width, activation=tf.nn.relu)

    logging.info("Building linear output...")
    # <float32>[batch_size,1]
    predictions = tf.layers.dense(output_layer, 1, bias_initializer=tf.constant_initializer(0.15))
    # <float32>[batch_size]
...
def model_fn_builder(
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf_estimator.ModeKeys.TRAIN:
            ...
            logging.info(f"**** Initializing from {init_checkpoint} ****")
            ...
            if use_tpu:
                ...
                output_spec = tf_estimator.EstimatorSpec(mode=mode, loss=total_loss, train_op=train_op)

        elif mode == tf_estimator.ModeKeys.EVAL:
            if use_tpu:
                eval_metrics = (metric_fn, [per_example_loss, pred, scores])
                output_spec = tf_estimator.TPUEstimatorSpec(mode=mode, loss=total_loss, eval_metric=eval_metrics)
            else:
                output_spec = tf_estimator.EstimatorSpec(
                    mode=mode, loss=total_loss, eval_metric_ops=metric_fn(per_example_loss, pred, scores)
                )

        elif mode == tf_estimator.ModeKeys.PREDICT:
            output_spec = tf_estimator.EstimatorSpec(mode=mode, predictions={"predictions": pred})
...
def ranking_loss(predictions, labels):
    ...
    ii, jj = tf.meshgrid(tf.range(0, tf.shape(predictions)[0]), tf.range(0, tf.shape(predictions)[0]), indexing="ij")
    ...
    per_example_loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(score_diffs), logits=score_diffs)
...
def concat_tensors(predictions, ratings, sources=None):
    ...
    concat_predictions_value, concat_predictions_update = metrics.streaming_concat(predictions)
    ...
    return (
        ...
        tf.group(concat_predictions_update, concat_labels_update, concat_sources_update),
    )
...
def kendall_tau_metric(predictions, ratings, weights=None):
    ...
    with tf.variable_scope("kendall_tau"):
        concat_predictions_value, concat_labels_value, update_op = concat_tensors(predictions, ratings)
        metric_value = tf.reshape(
            tf.numpy_function(_kendall_tau, [concat_predictions_value, concat_labels_value], tf.float32), shape=[]
        )
...
def input_fn_builder(tfrecord_file, seq_length, is_training, batch_size, drop_remainder=True):
    ...
    def input_fn(params):  # pylint: disable=unused-argument
        ...
        if n_files > 1:
            logging.info(f"Found {str(n_files)} files matching {tfrecord_file}")
...
def _serving_input_fn_builder(seq_length):
    ...
    if tf.executing_eagerly():
        assert not FLAGS.dynamic_seq_length, "Training with `dynamic_seq_length is not supported in Eager mode."
        name_to_features = {
...
def run_finetuning(
    ...
    logging.info(f"Train batch size: {str(FLAGS.batch_size)}, eval batch size: {str(FLAGS.eval_batch_size)}")
    logging.info(f"Train data: {train_tfrecord}")
    ...
    if isinstance(dev_tfrecord, str):
        logging.info(f"Validation data: {dev_tfrecord}")
        ...
        for i in range(1, len(dev_tfrecord)):
            ...
            eval_name = multi_eval_names[i] if multi_eval_names and len(multi_eval_names) > i else "eval_%s" % i
            additional_eval_specs.append(
                tf_estimator.EvalSpec(name=eval_name, input_fn=additional_dev_input_fn, steps=FLAGS.num_eval_steps)
            )
            logging.info("len(additional_eval_specs): ", len(additional_eval_specs))
    else:
        raise ValueError(f"dev_tfrecord can only be a string or list: {dev_tfrecord}")
