...
def get_bleurt_params_from_flags_or_ckpt():
    ...
    if FLAGS.init_bleurt_checkpoint:
        logging.info(f"Reading paramter from BLEURT checkpoint: {FLAGS.init_bleurt_checkpoint}")
        ...
        if FLAGS.max_seq_length:
            logging.warning("Overriding `max_seq_length`. This could have unintended consequences.")
            max_seq_length = FLAGS.max_seq_length

    else:
        logging.info("Reads parameters from flags.")
        assert (FLAGS.vocab_file and FLAGS.do_lower_case is not None) or FLAGS.sentence_piece_model, (
...
def read_bleurt_config(path):
    ...
    assert tf.io.gfile.exists(path), f"Could not find BLEURT checkpoint {path}"
    config_path = os.path.join(path, CONFIG_FILE)
    assert tf.io.gfile.exists(config_path), (
        f"Could not find BLEURT config file {config_path}. Are you sure {path} is a valid checkpoint?"
    )
    ...
    for k in bleurt_config:
        ...
        if k.endswith("_file") or k.endswith("_dir"):
            ...
            assert tf.io.gfile.exists(fname), f"File {fname} missing."
...
def finalize_bleurt_checkpoint(tf_export_path):
    ...
    tf.io.gfile.copy(bleurt_params["bert_config_file"], bert_config_file, overwrite=True)
    ...
    if bleurt_params["vocab_file"]:
        ...
        tf.io.gfile.copy(bleurt_params["sp_model"] + ".model", sp_copy_loc, overwrite=True)
        vocab_copy_loc = os.path.join(tf_export_path, "sent_piece.vocab")
        tf.io.gfile.copy(bleurt_params["sp_model"] + ".vocab", vocab_copy_loc, overwrite=True)
        vocab_val = None
