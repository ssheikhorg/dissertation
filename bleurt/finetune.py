...
from bleurt import encoding, model
...
flags.DEFINE_string(
    ...
    None,
    "Target file where the serialized train set will be created. Will use a temp file if None.",
)

flags.DEFINE_string(
    "serialized_dev_set",
    None,
    "Target file where the serialized dev set will be created. Will use a temp file if None.",
)
...
def run_finetuning_pipeline(train_set, dev_set, run_in_lazy_mode=True):
    ...
    logging.info(f"Exporting BLEURT checkpoint to {export_dir}.")
