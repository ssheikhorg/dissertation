...
from transformers import AutoModel, AutoModelForCausalLM, AutoTokenizer, Trainer, TrainingArguments, pipeline
...
class HallucinationEvaluator:
    ...
    def semantic_similarity(self, source: str, generated: str) -> dict:
        ...
        cos_sim = self.similarity_model.cosine_similarity(
            source_embedding.unsqueeze(0), gen_embedding.unsqueeze(0)
        ).item()
...
async def compare_models(model_names: list[str], prompts: list[dict], mitigation: str = None) -> dict:
    """Compare multiple models on the same set of prompts with mitigation support"""
    if not model_names:
